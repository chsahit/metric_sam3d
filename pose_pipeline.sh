#!/usr/bin/env bash
#
# This script runs stages 2-4 of the metric_sam3d pipeline (no mesh generation).
# Expects pre-generated meshes and masks in mesh_folder.
#
...
if [ -z "${CAPTURE_FOLDER}" ] || [ -z "${MESH_FOLDER}" ] || [ -z "${OUTPUT_FOLDER}" ]; then
    echo "Usage: $0 [--device|-d DEVICE] <capture_folder> <mesh_folder> <output_folder>"
    echo "Example: $0 --device 0 captures/tab2 meshes/tab2 outputs/test"
    echo ""
    echo "Options:"
...
python prepare_data_for_registration.py --capture_folder "${CAPTURE_FOLDER}" --mesh_folder "${MESH_FOLDER}"
